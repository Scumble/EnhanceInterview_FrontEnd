...
export class UserConnection {
	...
	remoteStreamSub: BehaviorSubject<MediaStream>;
	...
	constructor(user: IUser, isCurrentUser: boolean, rtcConnection: RTCPeerConnection) {
		...
		this.remoteStreamSub = new BehaviorSubject<MediaStream>(undefined);
		this.streamObservable = this.streamSub.asObservable();
		this.remoteStreamObservable = this.remoteStreamSub.asObservable();
	}
...
export class WebRtcSignalRService {
	...
	constructor() {
		this._hubConnection = new signalR.HubConnectionBuilder()
			.withUrl(`${environment.signalRUrl}/rtcSignal`)
			.build();
		...
		this._hubConnection.on('receiveSignal', async (user: string, signal: string) => {
	...
	public join(userName: string, room: string) {
		...
		this._connections[this.currentConnectionId] = new UserConnection(
			{ userName: userName, connectionId: this.currentConnectionId }, true, undefined);
	...
	public async hangUp() {
		...
		console.log(this.currentMediaStream.getVideoTracks()[0]);
	}
	...
	private async getUserMediaInternal(): Promise<MediaStream> {
		try {
			...
			console.log(this.currentMediaStream);
	...
	private async initiateOffer(acceptingUser: IUser) {
		...
		connection.remoteStreamSub.next(localStream);
	...
	public async closeVideoCall(partnerClientId: string) {
